...
showlogo=False
#-------------------------------------------------------------------------------    
#   These variables and functions are for lighting up the "bowtie" display on the SenseHat
...
#----------------------------------------------------------------
# Turn on the display with an IBM "Q" logo
#----------------------------------------------------------------
...
def startIBMQ():
    ...
    if p==200:
        #IBMQ = IBMQuantumExperience(myIBMQtoken)
        IBMQ.load_accounts()
    else:
        exit()
#-------------------------------------------------------------------------------
   ...
   if p==200:
       ...
       if Q.status().status_msg == 'active':
           ...
           while not qdone:
               ...
               #print(runcounter,": ",result.status)
               #if result.status == 'COMPLETED':
                   qdone = True
           result=qjob.result()     # get the result
