...
class SenseHat(object):
    ...
    # ------------- Don't start emulator GUI, we'll  Create Fake Frame Buffer 
    ...
    # ----------  Setup Filename for 128 Byte file as faux frame buffer

        home_dir = os.path.expanduser('~')
        hat_dir = home_dir + '/.sensehat'
        if not os.path.exists(hat_dir):
            os.makedirs(hat_dir)
        self._fb_device = hat_dir + '/.FB_Device'
        SENSE_HAT_FB_PIX = [int(0),int(0),int(0)]
        SENSE_HAT_FB_BLACK = [SENSE_HAT_FB_PIX] * 64
        ...
        # --- Initialize faux Frame Buffer file

        with open(self._fb_device, 'wb') as f:
            map = self._pix_map[self._rotation]
            for index, pix in enumerate(SENSE_HAT_FB_BLACK):
