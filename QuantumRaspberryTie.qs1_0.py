#----------------------------------------------------------------------
#     QuantumRaspberryTie.qk1_0
#       by KPRoche (Kevin P. Roche) (c) 2017,2018,2019,2020,2021,2022.2024
#
#	  NEW RELEASE April 2024 to accomodate the official release of Qiskit 1.0
#     using new libraries and call techniques;
...
print(IBMQVersion)
print("     .....local emulator (fakeManila)")
...
if UseLocal:
    print (".... enabling local simulator")#importing Aer to use local simulator")
    ...
    if not UseLocal:
        ...
        if p==200:
            if (IBMQP_Vers >= 1):   # The new authentication technique with provider as the object
                provider0=QiskitRuntimeService()
                try:
                    Q=provider0.backend(backendparm)
                except:
                    Q=provider0.backend(backend)
                else:
                    interval = 300
